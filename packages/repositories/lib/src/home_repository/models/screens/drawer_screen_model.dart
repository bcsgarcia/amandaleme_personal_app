...
class DrawerScreenModel {
  ...
  factory DrawerScreenModel.fromJson(Map json) {
    return DrawerScreenModel(
      ...
      informations:
          List<InformationModel>.from(json['companyTipsInformation'].map((item) => InformationModel.fromJson(item))),
      posturalPatterns: List<PosturalPatternModel>.from(
          json['companyPosturalPatterns'].map((item) => PosturalPatternModel.fromJson(item))),
      partnerships:
          List<PartnershipModel>.from(json['companyPartnerships'].map((item) => PartnershipModel.fromJson(item))),
    );
  }
